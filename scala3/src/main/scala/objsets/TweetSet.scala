...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val tmp = right.filterAcc(p, left.filterAcc(p, acc))
  ...
  def mostRetweeted: Tweet = {
    def helper(tweetSet: TweetSet): Tweet = if (tweetSet.isEmpty) elem else tweetSet.mostRetweeted
    elem.max(helper(left)).max(helper(right))
  }

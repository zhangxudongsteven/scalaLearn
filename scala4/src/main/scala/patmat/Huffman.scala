...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: tail => (makeCodeTree(left, right) :: tail).sortBy(x => weight(x))
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def helper(sub: CodeTree, bits: List[Bit]): List[Char] = sub match {
      case Leaf(c, _) if bits.isEmpty => List(c)
      case Leaf(c, _) if bits.nonEmpty => c :: helper(tree, bits)
      case Fork(left, right, _, _) if bits.head == 0 => helper(left, bits.tail)
      case Fork(left, right, _, _) if bits.head == 1 => helper(right, bits.tail)
    }
    helper(tree, bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    // 将每个字母对应的 List[Bit] 合并成一个 List[Bit]
  ...
  def codeBits(table: CodeTable, char: Char): List[Bit] =
    if (table.isEmpty) throw new Exception("char not found") else
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def append(b: Bit)(code: Code): Code = (code._1, b :: code._2)
    // 此处用 :: 则返回 List(List, List)
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def helper(text: List[Char]): List[Bit] =
      if (text.isEmpty) List() else codeBits(table, text.head) ::: helper(text.tail)
    helper(text)
  }
